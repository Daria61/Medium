const Todolist =({tasks, setTasks, deleteList, edit})=>{
    return(
        <div className='mt-3 pt-3 border-top '>
            {tasks.map((a) => {
              return(
                <div className='d-flex row gap-2 border rounded align-items-center p-2 m-1'> 
                  <input type="checkbox" checked={a.isDone}   className="col"/>
                  <p className='m-0 col-4'>{a.input}</p>
                  <p className='m-0 col-2'>{ a.isCope ? <i class="bi bi-star-fill" ></i> : <i class="bi bi-star"></i>} </p>
